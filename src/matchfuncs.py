...
def sudden_mean(player1,player2,lanes, start_index,score):
    lane_index = start_index
    while True:
        res = (getMean(player1,lanes[lane_index]),
               getMean(player2,lanes[lane_index])) 
        if res[0]<res[1]:
            score[0] = score[0] + 1
        if res[1]<res[0]:
            score[1] = score[1] + 1
...
def sudden_log(player1,player2,lanes, start_index,score,resfunc):
    lane_index = start_index
    while True:
        res = (resfunc(player1,lanes[lane_index]),
               resfunc(player2,lanes[lane_index])) 
        if res[0]<res[1]:
            score[0] = score[0] + 1
...
def match_log(player1,player2,lanes, lane,resfunc):
    ...
    while True:
        ...
        lane_index = (lane_index+1)%len(lanes)
        if (lanes_left == 0):
            return sudden(player1,player2,lanes, lane_index,score)+['SD'],score_lanes
